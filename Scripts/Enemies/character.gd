...
func receive_dmg(incoming_dmg : float, defensive_value : float) -> void:
	if _is_dead:
		return
	# recibido = base * (1 - (defensivo / (defensivo + k))
	var total_damage = incoming_dmg * (1 - (defensive_value / (defensive_value + character_attributes.defense_effectiveness)))
